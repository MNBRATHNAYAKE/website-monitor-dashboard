...
function App() {
        ...
        prev.map((m) =>
          m._id === monitor._id
            ? {
                ...
                history: [...m.history.slice(-9), { timestamp: new Date().toISOString(), responseTime: data.responseTime }],
              }
            : m
        )
      );
        ...
        prev.map((m) =>
          m._id === monitor._id
            ? {
                ...
                status: "down",
                usedFallback: false,
                history: [...m.history.slice(-9), { timestamp: new Date().toISOString(), responseTime: 0 }],
              }
            : m
        )
      );
  ...
  const addMonitor = () => {
    ...
    const newMonitor = { _id: Date.now().toString(), name, url, status: "checking", history: [] };
  ...
  const displayedMonitors = monitors
    ...
    .sort((a, b) => (sortBy === "name" ? a.name.localeCompare(b.name) : a.status.localeCompare(b.status)));
    ...
    <div className="app">
      ...
      <div className="dashboard">
        {displayedMonitors.map((m) => {
          ...
          const lineColor = m.status === "up" ? "#4caf50" : m.status === "down" ? "#f44336" : "#ffeb3b";
          ...
          return (
            <div key={m._id} className="monitor-card">
              ...
              <p>
                ...
                <span
                  className={
                    m.status === "up" ? "status-up" : m.status === "down" ? "status-down" : "status-checking"
                  }
                >
                  {m.status}
                </span>
              ...
              <ResponsiveContainer width="100%" height={100}>
                <LineChart data={m.history}>
                  <Line type="monotone" dataKey="responseTime" stroke={lineColor} strokeWidth={2} dot={false} />
